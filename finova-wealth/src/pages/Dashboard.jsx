  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-surface-900">Good afternoon, Prateek 👋</h1>
...
=======
        ...
        <div className="space-y-6 lg:col-span-1">
          <div className="bg-white rounded-2xl p-6 shadow-card border border-surface-100 h-full">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2"><Brain className="w-5 h-5 text-primary-600" /><h3 className="font-semibold text-surface-900 text-sm">AI Insights</h3></div>
              <Link to="/ai-recommendations" className="text-xs text-primary-600 font-medium">View all →</Link>
            </div>
            <div className="space-y-4">
              {aiRecommendations.slice(0, 3).map(r => (
                <div key={r.id} className="p-4 rounded-xl bg-primary-50/50 border border-primary-100">
                  <p className="text-sm font-semibold text-surface-900">{r.title}</p>
                  <p className="text-xs text-surface-500 mt-1 leading-relaxed">{r.impact} · {r.confidence}% confidence</p>
                  <Link to="/ai-recommendations" className="text-[10px] text-primary-600 font-bold mt-2 inline-block uppercase tracking-wider">Explore Insight →</Link>
                </div>
              ))}
            </div>
